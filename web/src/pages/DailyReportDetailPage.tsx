    ...
    <Card title={`日報 ${report.businessDate}`}>
      ...
      <div className="settings-section-panel" style={{ marginBottom: "1rem" }}>
        ...
        <p className="settings-hint">
          ペアはこの日報内で変更できます。変更後に追加する運行の乗務記録簿「同伴乗務員名」には新しいペアが入り、それ以前の運行は登録時の名前のままです。随伴車を変える場合は、客車担当と随伴車の組み合わせごとに日報が分かれるため、新しい日報を作成してください（一覧で同じ事業日・担当・随伴の組が重ならないようにします）。
        </p>
        <Err msg={sessionErr} />
        <div className="settings-form">
          <label>ペア（乗務員）</label>
