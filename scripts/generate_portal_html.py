    ...
    <script>
      (function () {{
        ...
        function buildLiveIndexFromApi(byKey) {{
          ...
          if (!byKey || typeof byKey !== "object") return index;
        ...
        function showLiveSlot(slot, live) {{
          ...
          slot.className = LIVE_SLOT_VISIBLE;
        ...
        async function loadLiveInfo() {{
          try {{
            ...
            liveIndex = buildLiveIndexFromApi(data.by_key);
            if (PORTAL_LIVE_DEBUG) {{
              console.log("[portal-live] インデックスキー数", Object.keys(liveIndex).length);
            }}
            return true;
          }} catch (err) {{
