...
<#
  Push origin/main from this daiko repository root, then on VPS: git pull in DAIKO_VPS_PATH (clone root = app root), migrate, build, restart daiko-app.
...
$remote = "set -e; cd $remotePath; git pull; if [ -f package-lock.json ]; then npm ci; else npm install; fi; npx prisma migrate deploy; npx prisma generate; npm run db:seed; npm run build; sudo systemctl restart $service; sleep 2; curl -sS http://127.0.0.1:3001/health"
